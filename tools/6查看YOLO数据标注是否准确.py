...
labels = ['MD', 'LS-N6', 'LS-SZ', 'LM-6J', 'LS-6J','LS-6J']
color_list = [(0, 0, 255), (255, 0, 0), (0, 255, 0), (0, 255, 255), (0, 255, 255), (0, 255, 255)]
file_dir = '/home/mengxianchi/train_img/AK_CLW/data/files/dataset/val'
img_dir = file_dir+'/images'
yolo_txt_dir = file_dir+'/labels'
result_dst_dir = file_dir+'/result'
scale_percent = 100
# rates that represent the imgs of all datasets
# 1 for all imgs, 0.5 for half of the imgs
check_rate = 0.5
random_check = False
if not os.path.isdir(result_dst_dir):
    os.makedirs(result_dst_dir)
...
def draw_label_rec(img, label_index, label_info_list, img_name):
    ...
    ymax = int(y_center + h * img_height / 2)
    ymin = int(y_center - h * img_height / 2)
...
def main():
    ...
    for index, img_name in enumerate(img_name_list):
        ...
        if index >= check_max_times:
            return
        print("**check img : {0} **".format(img_dir+'/'+ img_name))
        # Open IMG
        ...
        cv.imwrite(result_dst_dir+'/'+img_name,resized_src)
        # show the result
